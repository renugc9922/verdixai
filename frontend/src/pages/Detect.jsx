...
import { fileToDataUrl, supportedDemoFilenames } from '../utils/verdix'
      ...
      <main className="relative z-10 mx-auto grid w-full max-w-7xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:py-10">
        ...
        <motion.section initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.05 }} className="rounded-[2rem] border border-white/10 bg-white/6 p-5 shadow-[0_25px_80px_rgba(0,0,0,0.24)] backdrop-blur-2xl sm:p-6">
          <div className="rounded-[1.75rem] border border-white/10 bg-[#07140f]/85 p-5 sm:p-6">
            ...
            <div className="mt-5 rounded-[1.5rem] border border-white/10 bg-white/5 p-4 backdrop-blur-md">
              <p className="text-sm uppercase tracking-[0.24em] text-emerald-100/55">Supported demo filenames</p>
              <p className="mt-2 text-sm leading-6 text-emerald-100/65">Offline demo works only with supported sample filenames when AI service is unavailable.</p>
              <p className="mt-3 text-sm leading-6 text-emerald-100/55">{supportedDemoFilenames.join(', ')}</p>
            </div>
          </div>
        </motion.section>
      </main>
